...
export default function TasksPage() {
  ...
  return <>
    ...
    <div className="card mb-5 grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-[2fr_repeat(3,1fr)]"><input className="field" placeholder="搜索客户姓名或公司" value={query} onChange={e => setQuery(e.target.value)} /><Filter label="全部状态" value={status} set={setStatus} values={statuses} /><Filter label="全部客户类型" value={type} set={setType} values={["经销商", "代理商", "终端工厂", "系统集成商/工程公司", "服务商", "贸易商", "制造商/同行", "行业联系人", "无法判断", "设备集成商", "工程项目方", "其他"]} /><Filter label="全部渠道" value={channel} set={setChannel} values={["LinkedIn", "Facebook", "Email", "WhatsApp"]} /></div>
