...
export function buildFollowUpGenerationRequest(task: Task, input: { replyGoal: ReplyGoal; customReplyGoal: string; tone: FollowUpTone; businessFacts: string; latestCustomerReply?: string }): FollowUpGenerationRequest {
  ...
  return {
    ...
    analysis: {
      ...
      confirmedFacts: [...(task.analysis.confirmedFacts || [])],
      reasonableInferences: [...(task.analysis.reasonableInferences || task.analysis.inferences?.map(item => item.content) || [])],
      unknownInformation: [...(task.analysis.unknownInformation || [])],
    },
